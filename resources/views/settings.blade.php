<div>
    ...
    <x-form class="flex flex-col lg:flex-row gap-4 lg:gap-6" wire:submit="save">
        ...
        <div class="flex-1 space-y-6">
            ...
            <flux:input wire:model="profilePicture" label="Change Profile Picture" type="file" />
            <div class="flex justify-end items-center gap-4">
                @if($profileSuccess)
                    <flux:text x-data="{ show: true }" x-init="setTimeout(() => show = false, 10000)">{{ $profileSuccess }}</flux:text>
                @endif
                @if($profilePictureUrl)
                    <flux:button type="submit" variant="subtle" wire:click.prevent="removeProfilePicture">Remove Profile Picture</flux:button>
                @endif
